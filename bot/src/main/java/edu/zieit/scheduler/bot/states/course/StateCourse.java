...
public class StateCourse extends State {
    ...
    public void activate(ChatSession session) {
        ...
        SubscriptionStudent subs = subsService.getStudentSubs(session.getChatId());

        if (subs != null) {
            ScheduleService service = session.getChatManager().getBot().getScheduleService();
            Schedule schedule = service.getStudentSchedule(subs.getScheduleKey());

            if (schedule instanceof StudentSchedule course) {
